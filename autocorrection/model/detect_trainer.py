...
class ModelDetectTrainer:
    ...
    def get_loss(self, detection_outputs, label_errors):
        return self.loss_detection(detection_outputs.reshape(-1, self.n_errors), label_errors.reshape(-1))
    ...
    def train_one_epoch(self):
        ...
        for idx, data in enumerate(self.train_loader):
            ...
            if self.add_char_level:
                ...
                detection_outputs, _ = self.model(error_ids, char_error_ids, batch_splits)
            elif self.is_bert:
                attention_masks = data['attention_masks'].to(device)
                detection_outputs, _ = self.model(error_ids, attention_masks, batch_splits)
            else:
                detection_outputs, _ = self.model(error_ids)
            loss = self.get_loss(detection_outputs, label_errors)
    ...
    def fit(self):
        ...
        for epoch in range(self.n_epochs):
            ...
            val_loss, val_acc_detect = self.evaluate_model()
            ...
            print(
                f"EPOCH:{epoch}---Detector val acc:{val_acc_detect}---Train "
                f"loss:{train_loss}---Val loss:{val_loss}---Time:{time.time() - start_time}"
            )
            if (val_acc_detect) > best_acc:
                self.save_model(epoch, history)
                best_acc = val_acc_detect
                
            print("Current acc:",val_acc_detect)
