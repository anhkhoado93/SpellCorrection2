...
from tone.model import Seq2SeqConcat, Encoder, Decoder
from tone.dataset import Seq2SeqDataset, PAD
from tone.alphabet import LEGAL, PUNCT, A_LIST, E_LIST, I_LIST, O_LIST, U_LIST, Y_LIST, D_LIST
from tone.params import DEVICE
from tone.lm import KenLM
...
class TonePredictor:
    def __init__(self, src_vocab_path, tgt_vocab_path, model_path, wlm_path,
        ...
        with open("tone/data/legal_vc.txt", 'r') as f:
            for line in f:
                vc = line.strip()
    ...
    def predict_line(self, text):
        lines_raw = text
        lines_prep = [self.preprocess(line_seq) for line_seq in lines_raw]
    ...
    def beam_lm(self, predicted_seq, predicted_values, line_raw, k=10, threshold=0.99):
    ...
    def match_tone(self, predicted, src):
        """
        Match case of source and added tone sentence: only add tone to word that have no tone from source
        """
        src_words = src.split(' ')
        predicted_words = predicted.split(' ')
        
        masking = [self.is_ascii(w) for w in src_words]
        
        result = []
        for i in range(len(predicted_words)):
            if not masking[i]:
                result.append(src_words[i])
            else:
                result.append(predicted_words[i])
        return ' '.join(result)
    
    def match_tone_lm(self, predicted, src):
        """
        Calculate combination of predicted and src sentence to select sentence with highest lm score
        """
        src_words = src.split(' ')
        predicted_words = predicted.split(' ')

        if len(predicted_words) == len(src_words):

            combinations = []
            for i in range(len(src_words)):
                if src_words[i] == predicted_words[i]:
                    combinations.append([src_words[i]])
                else:
                    combinations.append([src_words[i], predicted_words[i]])
            sents = [' '.join(words) for words in list(itertools.product(*combinations))]
            lm_scores = []
            for i in range(len(sents)):
                lm_scores.append({
                    'text': sents[i],
                    'score': self.score(sents[i].lower())
                })
            ...
            return self.match_tone(predicted, src)
...
#     wlm_path = "lm/vietnamese-lm.binary"
...
#     predictor = TonePredictor(src_vocab_path, tgt_vocab_path, model_path, wlm_path)
#     # predictor.predict(test_path, test_cleaned_path, out_path)
#     print(predictor.predict_line([text]))